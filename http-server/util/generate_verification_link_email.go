...
func GeneratePasswordResetLink(userUuid string) (string, string) {
	expires := time.Now().Add(constants.EMAIL_LINK_TIMEOUT).Unix()

	payload := fmt.Sprintf("password-reset:%s:%d", userUuid, expires)
	...
	return fmt.Sprintf("%s/paylist-reset-password?%s", os.Getenv("CLIENT_URL"), values.Encode()), signature
}

func ValidatePasswordResetLink(userUuid, expStr, sig string) bool {
	exp, err := strconv.ParseInt(expStr, 10, 64)
	if err != nil {
		return false
	}
	...
	payload := fmt.Sprintf("password-reset:%s:%d", userUuid, exp)
